...
# Obtém o handle do sensor ultrassônico
...
# Inicia a captura dos dados do sensor ultrassônico
...
try:
    while True:
        ...
        if returnCode == sim.simx_return_ok:
            # Controle dos motores baseado no sensor ultrassônico
            if front_detectionState and np.linalg.norm(front_detectedPoint) < 0.2:  # Obstáculo a menos de 0.5m
                ...
                # Gira o robô para evitar o obstáculo
               # turn_angle(clientID, left_motor_handle, right_motor_handle, -90)  # Gira 90 graus para a direita
                if np.linalg.norm(right_detectedPoint) < np.linalg.norm(left_detectedPoint):
                    lSeed = -0.15
                    rSeed = 0.15
                else:
                    lSeed = 0.15
